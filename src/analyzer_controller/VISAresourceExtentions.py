...
pyvisa.Resource.ext_clear_status = ext_clear_status
...
pyvisa.Resource.ext_check_error_queue = ext_check_error_queue
...
pyvisa.Resource.ext_wait_for_opc = ext_wait_for_opc
...
pyvisa.Resource.ext_error_checking = ext_error_checking
...
pyvisa.Resource.ext_query_bin_data_to_file = ext_query_bin_data_to_file
...
pyvisa.Resource.ext_send_pc_file_data_to_instrument = ext_send_pc_file_data_to_instrument
...
pyvisa.Resource.ext_copy_pc_file_to_instrument = ext_copy_pc_file_to_instrument
...
pyvisa.Resource.ext_write_with_stb_poll_sync = ext_write_with_stb_poll_sync
...
pyvisa.Resource.ext_query_with_stb_poll_sync = ext_query_with_stb_poll_sync
...
def ext_write_with_srq_sync(device, cmd, timeout_ms):
    device.query('*ESR?')
    full_cmd = cmd + ';*OPC'
    device.discard_events(pyvisa.constants.VI_EVENT_SERVICE_REQ, pyvisa.constants.VI_ALL_MECH)
    device.enable_event(pyvisa.constants.VI_EVENT_SERVICE_REQ, pyvisa.constants.VI_QUEUE)
    device.write(full_cmd)
    device.wait_on_event(pyvisa.constants.VI_EVENT_SERVICE_REQ, timeout_ms)
    device.disable_event(pyvisa.constants.VI_EVENT_SERVICE_REQ, pyvisa.constants.VI_QUEUE)
    device.query('*ESR?')

pyvisa.Resource.ext_write_with_srq_sync = ext_write_with_srq_sync
...
pyvisa.Resource.ext_query_with_srq_sync = ext_query_with_srq_sync
...
def ext_write_with_srq_event(device, cmd, handler):
    ...
    device.discard_events(pyvisa.constants.VI_EVENT_SERVICE_REQ, pyvisa.constants.VI_ALL_MECH)
    try:
        device.uninstall_handler(pyvisa.constants.VI_EVENT_SERVICE_REQ, handler)
    except pyvisa.UnknownHandler:
        pass

    device.install_handler(pyvisa.constants.VI_EVENT_SERVICE_REQ, handler)
    device.enable_event(pyvisa.constants.VI_EVENT_SERVICE_REQ, pyvisa.constants.VI_HNDLR, 0)
    device.write(full_cmd)


pyvisa.Resource.ext_write_with_srq_event = ext_write_with_srq_event
